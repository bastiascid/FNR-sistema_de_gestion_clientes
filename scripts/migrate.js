...
try {
  ...
  db.transaction(() => {
    numSheets.forEach(sheetName => {
      ...
      insertClientStmt.run(id, nombre, null, null, null, null, null);
      clientsImported++;
      ...
      for (let r = 12; r < data.length; r++) {
        ...
        if (credit > 0 || abono > 0 || (dateVal && typeof dateVal === 'number') || detail) {
          // If detail is "Saldo inicial", it might have a credit or abono
          // In some client sheets, "Saldo inicial" has a balance.
          
          // Use currentTransactionDate, fallback to date of the movement or general default
